...
export default function Sidebar({ open, onClose }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed lg:static z-40 top-0 left-0 h-full w-64 bg-ink text-white flex flex-col transition-transform duration-300 ${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
          }`}
      >
        ...
        <div className="px-3 pb-4 border-t border-white/10 pt-4">
          ...
          <button
            ...
            disabled={loggingOut}
            className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-sm font-medium text-white/60 hover:bg-white/5 hover:text-white transition-colors disabled:opacity-50"
          >
            <LogOut size={16} />
